...
output$to_do_table <- renderTable({
        if(!is.null(rv$to_do_df)){
                to_do_df_show <- rv$to_do_df
                ...
                to_do_df_show <- to_do_df_show %>% dplyr::select(area, item, to_do, to_due_days, due, done, frequency)
