rv <- list()
rv$DFk <- readRDS(file = file.path("/Users/jvb740/Downloads", "Faelles.rds"))
input <- list()
input$plotDate <- "2018-04-28"
...
if (dim(DFkPlot)[1] == 0) {
        ...
        dayTr <- dayPlotMock(plotDate, plotWidth = 10)
        ...
        if(any(DFkPlot$endDate > DFkPlot$startDate)){
                ...
                DFkPlot <- dplyr::filter(DFkPlot, Duration > 0) # otherwise crash when plotted
        }
        # --
        ...
        # use dayPlots()
        dayTr <- dayPlot(DFkPlot, plotDate = plotDate, plotWidth = 10)
        # --
