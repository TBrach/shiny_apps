...
colorValues <- c("Blood Pressure systolic" = cbPalette[1], "Blood Pressure diastolic" = "red", Pulse = "#440154FF", HbA1c = "#5DC863FF",
                 LDL = "#7F00FF") # "#006633"

# - Define Levels, Units, and Goals -
itemLevels <- c("Blood Pressure systolic", "Blood Pressure diastolic", "Pulse", "HbA1c", "LDL", "HDL", "VLDL", "Total cholesterol", "Triglycerides")
unitLevels <- c("mmHg", "mmHg", "min-1", "mmol/mol", "mmol/L", "mmol/L", "mmol/L", "mmol/L", "mmol/L")
names(unitLevels) <- itemLevels
# https://www.mayoclinic.org/tests-procedures/cholesterol-test/about/pac-20384601
itemGoals <- c(120, 80, 60, 32, 2.6, 1.5, 0.9, 4, 1.7)
names(itemGoals) <- itemLevels
# --
...
monthPlot <- function(PlotDF, plotDate, plotWidth = 5){
        ...
        if(nrow(DFkMonthPlot) != 0){
                ...
                Tr <- Tr +
                        ...
                        geom_rect(mapping = aes(xmin=startPosition, xmax=startPosition+Width, ymin=Hours, ymax=Hours+Duration/60, fill = Color), alpha=0.75) + 
